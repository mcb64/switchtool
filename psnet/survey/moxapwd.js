...
function computePass(raw_pass, challenge) {
  ...
  md = MD5(challenge);
  for (i = 0; i < raw_pass.length; i++) {
    q = ascii.lastIndexOf(raw_pass.charAt(i));
    p += q.toString(16);
   }
   for (i = 0; i < p.length; i++) {
    m = hex_h.indexOf(p.charAt(i));
    n = hex_h.indexOf(md.charAt(i));
    md5_pass += hex_h.charAt(xor(m,n));
   }
   return  md5_pass;
}

console.log(computePass(process.argv[2], process.argv[3]));
